...
// Add missing imports and constants
import { ethers } from 'ethers';

// Constants that need to be defined or imported
// Use import.meta.env for Vite or window.env for Create React App
const API_URL = import.meta.env?.VITE_API_URL || window.env?.REACT_APP_API_URL || 'http://localhost:3001';
const DEFAULT_FEES = {
  managementFee: 2,
  performanceFee: 20,
  entryFee: 1,
...
const CreatePortfolio = () => {
  ...
  const fetchUserPortfolios = async () => {
    ...
    try {
      console.log('Fetching portfolios for account:', account.toLowerCase());
      const response = await fetch(`${API_URL}/portfolios/user/${account.toLowerCase()}`);
      if (!response.ok) {
